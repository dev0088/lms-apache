...
class User extends CI_Controller {
    ...
    public function forumTable (){
        ...
        $columns = array(
           ...
           array( 'db' => 'customer_name', 'dt' => 2 ),array( 'db' => 'admin_id', 'dt' => 3 ),
           array( 'db' => 'users_id', 'dt' => 4 ),array( 'db' => 'form', 'dt' => 5 ),
           array( 'db' => 'status', 'dt' => 6 ),array( 'db' => 'html_file', 'dt' => 7 ),
           array( 'db' => 'form_id', 'dt' => 8 ), array( 'db' => 'vice_admin', 'dt' => 9 )
        );
        ...
        $where = '';
        if($type != 'admin') {
          $where = array("(users_id = '".$id."') OR (users_id <> '".$id."' AND status = 'actif') OR (users_id <> '".$id."' AND vice_admin = '".$id."')");
        }
        ...
        foreach ($output_arr['data'] as $key => $value) {
            ...
            if (!empty($output_arr['data'][$key][3])) {
              $result1 = $this->User_model->get_users($output_arr['data'][$key][3]);
              $admin_name = $result1[0]->name;
            }
            

            $html_file = $output_arr['data'][$key][7];
            $form_name = $output_arr['data'][$key][5];
            $form_id = $output_arr['data'][$key][8];
            $preview = '<a href="'.base_url().$html_file.'" class ="mClass" style="cursor:pointer;" target="_blank"><i class="fa fa-eye" ></i></a>';
            if($type == 'admin') {
              $edit = '<a href="'.base_url().$form_name.'/form/'.$form_id.'" class ="mClass"><i class="fa fa-pencil" data-id=""></i></a>';
              $admin = '<select onchange="changeForumstatus(this, '.$output_arr['data'][$key][0].');"><option hidden disabled selected value>- Statut -</option><option value="actif">Actif</option><option value="inactif">Inactif</option></select>';
            } else {
              if( ($output_arr['data'][$key][6] != 'actif') || ($id == $output_arr['data'][$key][9]) ){
                $edit = '<a href="'.base_url().$form_name.'/form/'.$form_id.'" class ="mClass"><i class="fa fa-pencil" data-id=""></i></a>';
              }
              else{
            ...
            $output_arr['data'][$key][3] = $admin_name;
            $output_arr['data'][$key][4] = $user_name;
            $output_arr['data'][$key][7] = $preview;
            $output_arr['data'][$key][8] = $edit;
            $output_arr['data'][$key][9] = $admin;
        }
