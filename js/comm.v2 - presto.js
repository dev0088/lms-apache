                    ...
                    if (divId.length > 0) {
                        ...
                        newNode.innerHTML = '<div class="col-sm-10 col-xs-10">\
                                                                        <div class="messages msg_receive">\
                                                                                <p>' + name + ' ' + extraData.referrer + '<br/>' + osName + ' ' + browerName + '</p>\
                                                                                \
                                                                        </div>\
                                                                </div>';
                    ...
                    if (room.extra.role == 'popup') {
                        ...
                        if (element.length > 0 && checkExists.length === 0) {
                            ...
                            setTimeout(function () {
                                ...
                                var str = {};
                                str.names = (agentNames) ? agentNames : guestName(extraData.sessionId);
                                if (lsRepUrl) {
...
//                            self.handleCallTermination();
                ...
                if (event.giveAllParticipants && videoConnection && videoConnection.isInitiator) {
                    var participants = [];
                    videoConnection.getAllParticipants().forEach(function (pid) {
                        participants.push({
                            pid: pid,
        ...
        if (queryString.broadcast) {
            ...
            if (queryString.isAdmin || localStorage.getItem('hasPrivileges')) {
                ...
                videoConnection.openOrJoin(roomId + '_video', function (isRoomExist, roomid, error) {
                    ...
                    if (!videoConnection.isInitiator) {
                        var initialStatus = videoConnection.dontCaptureUserMedia;
                        videoConnection.dontCaptureUserMedia = true;
                        // each user must create a separate room as well!
                        videoConnection.open(videoConnection.userid, function (isRoomOpened, roomid, error) {
                            if (error) {
                                console.error('open', error, roomid);
                                return;
                            }
                            videoConnection.dontCaptureUserMedia = initialStatus;
                            videoConnection.isInitiator = false;
                        });
                    }
                });
            } else {
        ...
        lsDesigner.addSyncListener(function (data) {
            if (queryString.token) {
                $.ajax({
                    ...
                    data: {'type': 'adddrawing', 'drawing': JSON.stringify(data), 'roomId': roomId}
                })
                        .done(function (data) {
//                            console.log(data);
                        })
